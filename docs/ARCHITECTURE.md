...
- src/game/visual-system.js: 캐릭터·몬스터·기지·포탑·발사체의 등급 해석, 고유 시각 서명, 캐시된 프로파일을 담당한다.
...
- 동적 entity는 painter sort한다. 방어 기지도 같은 깊이 큐에 포함해 90° 회전에서 가림 순서를 보존한다.
- 시각 등급 프로파일은 순수 함수+bounded Map cache로 관리하고 발사체는 최대 6개 합성 레이어만 사용한다.
- 모바일 DPR은 평상시 1.5로 제한하고, 고밀도 장면에서만 1.25로 제한한다.
- 적 22/45에서 캐릭터 LOD를 낮춘다.
- 적 18 이상·구조물 28 이상에서는 pressure LOD와 DPR 1.25 상한을 적용한다. 화면상 수 픽셀인 캐릭터는 등급 팔레트·코어·무기 방향으로, 구조물은 핵심 몸체·코어·총열로 축약하되 등급 재질·종류·조준 방향·발사체 형상을 유지한다.
